...
public final class RiverSkeleton {
     ...
     * @param blocksPerCoarseCell {@code 32 * latentCompression * scale}
     */
    public static List<Segment> toWorld(List<RiverNetwork.Reach> reaches, int windowWidth,
                                        int ci0, int cj0, int blocksPerCoarseCell) {
        List<Segment> out = new ArrayList<>(reaches.size());
        double half = blocksPerCoarseCell / 2.0;
        ...
        for (RiverNetwork.Reach reach : reaches) {
            ...
            double x0 = (cj0 + fromCol) * (double) blocksPerCoarseCell + half;
            double z0 = (ci0 + fromRow) * (double) blocksPerCoarseCell + half;
            double x1 = (cj0 + toCol) * (double) blocksPerCoarseCell + half;
            double z1 = (ci0 + toRow) * (double) blocksPerCoarseCell + half;
    ...
    public static Segment clipToTile(Segment s, int tileX, int tileZ, int size) {
        double dx = s.x1 - s.x0;
        double dz = s.z1 - s.z0;
        double xMax = tileX + size, zMax = tileZ + size;
